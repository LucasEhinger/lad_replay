...
static const TString adcData[nAdcSignals]  = {"ErrorFlag", "PulseTimeRaw", "PulseInt"}; //Todo: fixmePulseAmp changed to PulseInt for testing
	...
	if(signalNames[isignal] == "Adc") {
	  ...
	  for(UInt_t iadcdata = 0; iadcdata < nAdcSignals; iadcdata++) {
	    ...
	    if (adcData[iadcdata] == "PulseInt") {
	      adcPulseAmpName = new TString(*adcBaseName+"PulseInt");//TODO: changed from PulseAmp to PulseInt for testing. Change back to PulseAmp
  ...
  for (ievent = 0; ievent < nentries; ievent++) {
	...
	for(UInt_t isignal = 0; isignal < nSignals; isignal++) {
	  ...
	  if(signalNames[isignal] == "Adc") { 
		...
		if (adcData[iadcdata] == "PulseInt") //TODO: changed from PulseAmp to PulseInt for testing. Change back to PulseAmp
	  ...
	  if(signalNames[isignal] == "Adc") { 
		...
		for(UInt_t iadcdata = 0; iadcdata < nAdcSignals; iadcdata++) {
		  ...
		  for (Int_t iadchit = 0; iadchit < numAdcHits; iadchit++) {
		    ...
		    // Implement cuts
		    // if (adcErrorFlagCut || adcPulseAmpCut) continue; //TODO: fixme. remove the hard coding here and uncomment this line
		    // Loop over the TDC data objects
		    for(UInt_t itdcdata = 0; itdcdata < nTdcSignals; itdcdata++) {
			...
			// Implement cuts
			// if (adcAndTdcHitCut || adcTdcTimeDiffCut) continue; //TODO: fixme. remove the hard coding here and uncomment this line
