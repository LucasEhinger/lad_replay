...
const static double TdcTimeTWCorr_MAX = 1000.0; //Was originally 100.

void fitHodoCali_old(TString filename, Int_t runNUM, Bool_t cosmic_flag = kFALSE) {
  ...
  TString spec                        = "L";
  TString det                         = "ladhod";
  string pl_names[5]                  = {"000", "001", "100", "101", "200"};
  string side_names[2]                = {"GoodPos", "GoodNeg"};
  string nsign[2]                     = {"+", "-"};
  Int_t maxPMT[5]                     = {11, 11, 11, 11, 11};
  Int_t refPad[5]                     = {0, 11, 22, 33, 44}; // use as reference when counting bars up to 52
  ...
  TH1F *h1Hist_TWDiffTrkPos[PLANES]
                           [16]; // 1D hist of TW Corr. Dist - Track Position (width should be Hodo resolution) <------
    ...
    for (Int_t side = 0; side < SIDES; side++) {
      ...
      for (Int_t ipmt = 0; ipmt < maxPMT[npl]; ipmt++) {
        ...
        h2Hist_TW_UnCorr[npl][side][ipmt] =
            new TH2F(Form("TW_UnCorr PMT %s%d%s", pl_names[npl].c_str(), ipmt + 1, nsign[side].c_str()),
                     Form("PMT %s%d%s: UnCorr. (TDC - ADC) Pulse Time vs. ADC Pulse Amplitude ", pl_names[npl].c_str(),
                          ipmt + 1, nsign[side].c_str()),
                     600, 0, 420, 150, -70, -40);
        h2Hist_TW_Corr[npl][side][ipmt] =
            new TH2F(Form("TW_Corr PMT %s%d%s", pl_names[npl].c_str(), ipmt + 1, nsign[side].c_str()),
                     Form("PMT %s%d%s: Corr. (TDC - ADC) Pulse Time vs. ADC Pulse Amplitude ", pl_names[npl].c_str(),
                          ipmt + 1, nsign[side].c_str()),
                     600, 0, 420, 150, -70, -40);
        ...
        if (side == 0) // require ONLY one side, since a time diff between two pmts at each end is taken
        {
          ...
          h1Hist_TWAvg[npl][ipmt] = new TH1F(
              Form("Avg. Time: Paddle %s%d", pl_names[npl].c_str(), ipmt + 1),
              Form("Paddle %s%d: Time-Walk Corrected Average Time", pl_names[npl].c_str(), ipmt + 1), 100, 15, 50);

          h1Hist_TWAvg_CUT[npl][ipmt] = new TH1F(
              Form("Avg. Time CUT: Paddle %s%d", pl_names[npl].c_str(), ipmt + 1),
              Form("Paddle %s%d: Time-Walk Corrected Average (CUT)", pl_names[npl].c_str(), ipmt + 1), 100, 15, 50);

          h2Hist_TWDiff_v_TrkPos[npl][ipmt] = new TH2F(
              Form("DistDiff: Paddle %s%d", pl_names[npl].c_str(), ipmt + 1),
              Form("Paddle %s%d: Time-Walk Corr. Hit Dist vs. Hod Track Position", pl_names[npl].c_str(), ipmt + 1),
              160, -80, 80, 50, -20, 20);
          h2Hist_TW_Corr_v_TrkPos[npl][ipmt] = new TH2F(
              Form("TimeDiff: Paddle %s%d", pl_names[npl].c_str(), ipmt + 1),
              Form("Paddle %s%d: Time-Walk Corr. TimeDiff. vs. Hod Track Position", pl_names[npl].c_str(), ipmt + 1),
              160, -40, 40, 120, -6, 6);

          h1Hist_TWDiffTrkPos[npl][ipmt] = new TH1F(
              Form("DistDiff - Track: Paddle %s%d", pl_names[npl].c_str(), ipmt + 1),
              Form("Paddle %s%d: Time-Walk Corr. Hit Dist. - Hod Track Position", pl_names[npl].c_str(), ipmt + 1), 120,
              -60, 60);

          h2Hist_TWAvg_v_TrkPos[npl][ipmt] = new TH2F(
              Form("TimeAvg_v_Trk: Paddle %s%d", pl_names[npl].c_str(), ipmt + 1),
              Form("Paddle %s%d: Time-Walk Corr. TimeAvg. vs. Hod Track Position", pl_names[npl].c_str(), ipmt + 1),
              160, -40, 40, 120, 15, 60);
          ...
          for (Int_t ipmt = 0; ipmt < maxPMT[npl]; ipmt++) {
            ...
            if (TdcTimeTWCorr[npl][0][ipmt] < TdcTimeTWCorr_MAX && TdcTimeTWCorr[npl][1][ipmt] < TdcTimeTWCorr_MAX) {
              ...
              if (side == 0) // require only one side, as a time diff between the two ends of a paddle is take
              {
                ...
                if (npl == 0 || npl == 2) {
                  h2Hist_TWAvg_v_TrkPos[npl][ipmt]->Fill(
                      TrackYPos[npl], 0.5 * (TdcTimeTWCorr[npl][1][ipmt] + TdcTimeTWCorr[npl][0][ipmt]));
                }

                else if (npl == 1 || npl == 3) {
                  h2Hist_TWAvg_v_TrkPos[npl][ipmt]->Fill(
                      TrackXPos[npl], 0.5 * (TdcTimeTWCorr[npl][1][ipmt] + TdcTimeTWCorr[npl][0][ipmt]));
                }
                if ((((TdcTimeTWCorr[npl][0][ipmt] + TdcTimeTWCorr[npl][1][ipmt]) / 2.) > (Mean - nSig * StdDev)) &&
                  ...
                  if (npl == 0 || npl == 2) {
                    ...
                    h2Hist_TW_Corr_v_TrkPos[npl][ipmt]->Fill(
                        TrackYPos[npl], 0.5 * (TdcTimeTWCorr[npl][1][ipmt] - TdcTimeTWCorr[npl][0][ipmt]));
                    ...
                    hhodo_sigArr[npl][ipmt] =
                        h1Hist_TWDiffTrkPos[npl][ipmt]
                            ->GetStdDev(); // Get Paddle resolution stdDev to be used in sig. parameters
  ...
  for (Int_t npl = 0; npl < PLANES; npl++) {
    ...
    TWCorr_v_TrkPos_canv[npl] =
        new TCanvas(Form("TWCorrTime_v_Pos%d", npl), Form("2DTWCorr_Time, plane %s", pl_names[npl].c_str()), 1000, 700);
    TWDiff_v_TrkPos_canv[npl] =
        new TCanvas(Form("TWCorrDist_v_Pos%d", npl), Form("2DTWCorr_Dist, plane %s", pl_names[npl].c_str()), 1000, 700);

    TWAvg_canv[npl] = new TCanvas(Form("TWAvg_%d", npl), Form("TWAvg, plane %s", pl_names[npl].c_str()), 1000, 700);
    TWAvg_canv_2D[npl] =
        new TCanvas(Form("TWAvg2D_%d", npl), Form("TWAvg2D, plane %s", pl_names[npl].c_str()), 1000, 700);
    Diff_TWDistTrkPos_canv[npl] =
        new TCanvas(Form("Diff_TrkDist_%d", npl), Form("Diff_TrkDist, plane %s", pl_names[npl].c_str()), 1000, 700);
    ...
    for (Int_t side = 0; side < SIDES; side++) {
      ...
      TWUnCorr_canv[npl][side] =
          new TCanvas(Form("TWUnCorrCanv%d%d", npl, side),
                      Form("plane %s_%s", pl_names[npl].c_str(), side_names[side].c_str()), 1000, 700);
  ...
  for (Int_t ipmt = 0; ipmt < 16; ipmt++) {
    if (ipmt == 0) {
      outPARAM << hhodo_velArr[0][ipmt] << ", " << setw(15) << hhodo_velArr[1][ipmt] << ", " << setw(15)
               << hhodo_velArr[2][ipmt] << ", " << setw(15) << hhodo_velArr[3][ipmt] << fixed << endl;
    } else {
      outPARAM << setw(23) << hhodo_velArr[0][ipmt] << ", " << setw(15) << hhodo_velArr[1][ipmt] << ", " << setw(15)
               << hhodo_velArr[2][ipmt] << ", " << setw(15) << hhodo_velArr[3][ipmt] << fixed << endl;
    }
  } // end pmt loop
  ...
  for (Int_t ipmt = 0; ipmt < 16; ipmt++) {
    if (ipmt == 0) {
      outPARAM << hhodo_cableArr[0][ipmt] << ", " << setw(15) << hhodo_cableArr[1][ipmt] << ", " << setw(15)
               << hhodo_cableArr[2][ipmt] << ", " << setw(15) << hhodo_cableArr[3][ipmt] << fixed << endl;
    } else {
      outPARAM << setw(26) << hhodo_cableArr[0][ipmt] << ", " << setw(15) << hhodo_cableArr[1][ipmt] << ", " << setw(15)
               << hhodo_cableArr[2][ipmt] << ", " << setw(15) << hhodo_cableArr[3][ipmt] << fixed << endl;
    }
  }
  ...
  for (Int_t ipmt = 0; ipmt < 16; ipmt++) {
    if (ipmt == 0) {
      outPARAM << hhodo_sigArr[0][ipmt] << ", " << setw(15) << hhodo_sigArr[1][ipmt] << ", " << setw(15)
               << hhodo_sigArr[2][ipmt] << ", " << setw(15) << hhodo_sigArr[3][ipmt] << fixed << endl;
    } else {
      outPARAM << setw(26) << hhodo_sigArr[0][ipmt] << ", " << setw(15) << hhodo_sigArr[1][ipmt] << ", " << setw(15)
               << hhodo_sigArr[2][ipmt] << ", " << setw(15) << hhodo_sigArr[3][ipmt] << fixed << endl;
    }
  }
  ...
  for (Long64_t i = 0; i < nentries; i++) {
    ...
    if (cosmic_flag)
      betaCut = beta > betanotrack_low_cut && beta < betanotrack_hi_cut;
    Bool_t hodTrk = TrackXPos[0] < 200 && TrackYPos[0] < 200 && TrackXPos[1] < 200 && TrackYPos[1] < 200 &&
                    TrackXPos[2] < 200 && TrackYPos[2] < 200 && TrackXPos[3] < 200 && TrackYPos[3] < 200;
    ...
    if (x1_hit && y1_hit && x2_hit && y2_hit && hodTrk) {
      ...
      for (Int_t npl = 0; npl < PLANES; npl++) {
        ...
        for (Int_t bar = 1; bar <= maxPMT[npl]; bar++) {
          ...
          if (goodhit[npl]) {
            ...
            good_TW_neg[npl] =
                TdcTimeTWCorr[npl][1][bar - 1] - 2 * hhodo_cableArr[npl][bar - 1]; // IMPORTANT: Apply cable time
      ...
      if (cnt == 4 && ngood < evtNUM) {
        ...
        if (ngood == evtNUM)
          cout << " Reach limit of filling matrix " << endl;
  ...
  for (Int_t ipmt = 0; ipmt < 16; ipmt++) {
    if (ipmt == 0) {
      outPARAM << setw(8) << LCoeff[0][ipmt] << ", " << setw(15) << LCoeff[1][ipmt] << ", " << setw(15)
               << LCoeff[2][ipmt] << ", " << setw(15) << LCoeff[3][ipmt] << fixed << endl;
    } else {
      outPARAM << setw(23) << LCoeff[0][ipmt] << ", " << setw(15) << LCoeff[1][ipmt] << ", " << setw(15)
               << LCoeff[2][ipmt] << ", " << setw(15) << LCoeff[3][ipmt] << fixed << endl;
    }
  }
}
