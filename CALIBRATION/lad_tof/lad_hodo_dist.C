...
const int YP_NB = 80;                        // y-on-bar: 6 cm bins
...
const int ED_NB = 150;                        // back-plane edep (MeV)
const double ED_LO = 0., ED_HI = 150.;
const int DT_NB = 150;                        // t_back - t_front (ns)
const double DT_LO = -10., DT_HI = 30.;
...
void lad_hodo_dist(const char *dat_file = DEFAULT_DAT_FILE, const char *out_file = DEFAULT_OUT_FILE, int nthreads = 100,
  ...
  // ---------------------------------------------------------------
  const char *CACHE_VERSION = "hd_v1";
  ...
  auto draw_qcanvas = [&](const std::string &cname, const std::string &ctitle, TH2 *base_all, TH2 *base_trk,
    ...
    TCanvas *c = new TCanvas(cname.c_str(), ctitle.c_str(), 1800, 1000);
    c->Divide(3, 2);
    for (int p = 0; p < 6; ++p) {
      c->cd(p + 1);
      ...
      TLegend *lg = new TLegend(0.66, 0.74, 0.98, 0.92);
      lg->SetFillStyle(0);
      for (int s = 0; s < 3; ++s) {
        TH1D *h = reg[s][p];
        h->SetStats(0);
        ...
        h->SetMarkerSize(0.7);
        h->SetTitle((std::string(rlab[p]) + ";" + xtitle + ";# hits").c_str());
