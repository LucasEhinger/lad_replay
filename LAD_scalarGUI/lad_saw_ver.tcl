...
if {$detector == 0} {
    ...
    set layernames {"W0S0" "W0S1" "W1S0" "W1S1" "W2"}
} else {
...
#
	if {[regexp {([0-9]+) *([A-Z][0-9][0-9]) *([0-9]+)([A-Z]) *([0-9]+)} $line ID layer lable number orientation scalchan] } then {
...
proc Draw {scalchan layer number orientation error minnumber maxnumber}   {
  ...
  #	set Y0 [expr {920 - 28 * $number}]
    ...
    set X1 $X0
    set Y1 $Y0
    if {$orientation == "T" || $orientation == "L"} {incr Y1 -46} elseif {$orientation == "B" || $orientation == "R"} {incr Y1 46}
