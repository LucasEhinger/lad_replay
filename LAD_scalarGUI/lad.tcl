...
set detector 0
# Each plane shown as a row instead of column
set swapxy 0
...
#set compoffset(localhost) 0
#set clockchan(localhost) 113
#set ratecheck(localhost) 113
set complist {lad-vme1}
set compoffset(lad-vme1) 0
set clockchan(lad-vme1) 492
set ratecheck(lad-vme1) 493
...
set epics_bcm_name "IBC3H00CRCUR4"
...
proc startup {} {
    ...
    catch {wm title . "LAD Detector Rates"}
#    wm minsize . 1250 875
    if {$swapxy == 0} then {
      wm geometry . [expr [llength $layers]*200]x1200
    } else {
      wm geometry . 1200x[expr [llength $layers]*159]
    }
    wm geometry . +0+0
...
#	pack .$element.title -in .$element.titleframe
    if {$swapxy == 0} then {
	  ...
	  canvas .$element.$element -width 920 -background white -height 100
	}
#	pack .$element .$element.$element -side left -fill both -expand 1
	...
	pack .$element.title .$element.$element -side left -fill both -expand 1 -in .$element
	pack .$element -side top -fill both -expand 1
	}
	incr i
    }
...
#
#               1 5B 23
#               Plane Planename CounterOrientation Scalerchan
#
	if {[regexp {([0-9]+) *([A-Z][0-9][0-9]) *([0-9]+)([A-Z][A-Z]) *([0-9]+)} $line ID layer lable number orientation scalchan] } then {
	   #puts "$ID|$layer|$lable|$number|$orientation|$scalchan"
...
proc Draw {scalchan layer number orientation error minnumber maxnumber}   {
    global ID channel chLetter chNumber comp indexnumber waittime \
	   ...
	   Label_List  first_time current rate swapxy
    ...
    set hspace [expr {(28 * 32) / ($maxnumber-$minnumber+1)}]
    set Y0 75
  #	set Y0 [expr {920 - 28 * $number}]
    set X0 [expr {468 + ($number-$minnumber - ($maxnumber-$minnumber)/2)*$hspace}]
    }
    set X1 $X0
    set Y1 $Y0
    if {$swapxy ==0} then {
        if {$orientation == "UF" || $orientation == "DF"} {incr X1 -46} elseif {$orientation == "UD" || $orientation == "DD"} {incr X1 46}
        if {$orientation == "UF" || $orientation == "UD"} {incr Y1 15} elseif {$orientation == "DF" || $orientation == "DD"} {incr Y1 -15}
	if {$orientation == "SG"} {incr X1 46}
    } else {
        if {$orientation == "UF" || $orientation == "DF"} {incr X1 -46} elseif {$orientation == "UD" || $orientation == "DD"} {incr X1 46}
        if {$orientation == "UF" || $orientation == "UD"} {incr Y1 15} elseif {$orientation == "DF" || $orientation == "DD"} {incr Y1 -15}
    }
...
#	    set Display_Number [expr round([lindex $difference($comp) [expr $scalchan - 1]] * $TimeConstant) / 1000.0]
	    set rounderr [catch {set Display_Number [expr round($rate([expr $scalchan]))/1000.0]}]
	    if {$rounderr} {
