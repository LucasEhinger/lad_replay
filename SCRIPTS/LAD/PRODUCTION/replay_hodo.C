// Only replay's LAD Hodo detector
// Currently set up to replay Hodo data from the hall (not ESB)
// #include "../../LAD/LAD_link_defs.h". Leave this line commented. Used for debugging purposes.
void replay_hodo(Int_t RunNumber = 0, Int_t MaxEvent = 0) {
  ...
  analyzer->SetOdefFile("DEF-files/LAD/PRODUCTION/lstackana_production_hodo.def");
  // Define cuts file
  analyzer->SetCutFile("DEF-files/LAD/PRODUCTION/CUTS/lstackana_production_cuts_hodo.def"); // optional
