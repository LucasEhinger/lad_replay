...
void lad_histos_MT(
  ...
  // Delete final histograms to free memory
  // for (auto &pair : finalHistograms1D) {
  //   delete pair.second;
  // }
  // finalHistograms1D.clear();

  // for (auto &pair : finalHistograms2D) {
  //   delete pair.second;
  // }
  // finalHistograms2D.clear();
  // std::map<TString, TH1D *> finalHistograms1D;
  // std::map<TString, TH2D *> finalHistograms2D;
  ...
  //     if (branchPair.second > NDDATA_MAX) {
  //       branchPair.second = NDDATA_MAX;
  //       std::cout << "Warning: NData for " << branchPair.first << " exceeds maximum limit. Clamping to " <<
  //       NDDATA_MAX
  //                 << std::endl;
  //     }
  //   }
